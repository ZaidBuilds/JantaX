    ...
    <div className="container" style={{ padding: '2rem 1.25rem' }}>
      <div style={{ maxWidth: 800, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', gap: '0.75rem', marginBottom: '1rem', flexWrap: 'wrap' }}>
          ...
          <button onClick={() => setShowFilters(v => !v)} style={{ padding: '0.6rem 0.75rem', borderRadius: 10, border: '1px solid #e2e8f0', background: showFilters ? 'var(--color-primary)' : '#fff', color: showFilters ? '#fff' : 'inherit', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '0.35rem', fontSize: '0.85rem' }}>
            <Filter size={14} /> Filters
          </button>
        </div>

        {showFilters && (
          <div style={{ display: 'flex', gap: '1rem', marginBottom: '1rem', flexWrap: 'wrap', padding: '1rem', background: '#f8fafc', borderRadius: 10 }}>
            <div>
            ...
            {(level !== 'all' || management !== 'all') && (
              <button onClick={() => { setLevel('all'); setManagement('all'); }} style={{ alignSelf: 'flex-end', padding: '0.45rem 0.75rem', border: '1px solid #e2e8f0', borderRadius: 8, background: '#fff', cursor: 'pointer', fontSize: '0.78rem', color: '#ef4444', fontWeight: 600 }}>
                Clear Filters
              </button>
            )}
          </div>
...
function SchoolSearchResultCard({ school, onView }: { school: SchoolRecord; onView: () => void }) {
  ...
  return (
    ...
    >
      <div style={{ width: 56, height: 56, borderRadius: 10, background: '#cbd5e1', overflow: 'hidden', flexShrink: 0 }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ display: 'flex', gap: '0.4rem', marginTop: '0.4rem', flexWrap: 'wrap' }}>
          {school.managementType !== 'Government' && (
            <span style={{ fontSize: '0.65rem', padding: '2px 6px', borderRadius: 4, background: '#f5f3ff', color: '#7c3aed', fontWeight: 600 }}>{school.managementType}</span>
          )}
          ...
          {school.totalCheckIns > 0 && (
            <span style={{ fontSize: '0.65rem', padding: '2px 6px', borderRadius: 4, background: '#f8fafc', color: '#64748b', fontWeight: 600 }}>
