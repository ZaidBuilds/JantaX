  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-4">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="bg-[#D43F33] text-white px-2 py-0.5 text-[10px] font-bold uppercase">
                Defect Liability Layer
              </span>
            </div>
            <h2 className="text-2xl sm:text-3xl font-black uppercase tracking-tight text-[#1A1A1A] mt-1.5">
              {currentWard?.name} (Ward #{currentWard?.number}) • {currentWard?.zone}
            </h2>
            <p className="text-xs font-serif text-[#1A1A1A]/80 mt-0.5">
              Dominant Contractor: <strong className="text-[#1A1A1A] underline font-sans font-bold">{currentWard?.dominantContractor.name}</strong> ({currentWard?.dominantContractor.sharePercent}% ward contracts)
            </p>
          </div>

          {/* Ward Switcher */}
          <div className="flex items-center gap-2 self-start lg:self-end">
            <span className="text-[10px] font-mono font-bold uppercase opacity-60">Ward:</span>
            <select
              ...
              className="bg-[#FAFAFA] border-2 border-[#1A1A1A] text-xs font-bold text-[#1A1A1A] px-3 py-1.5 focus:outline-none cursor-pointer uppercase"
            >
              {wards.map((w) => (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-4 pt-4 border-t border-black/10">
          ...
          <div>
            <select
              ...
              className="w-full bg-[#FAFAFA] border-2 border-[#1A1A1A] px-3 py-2 text-xs font-bold text-[#1A1A1A] focus:outline-none focus:bg-white cursor-pointer uppercase"
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
        ...
        <div className="lg:col-span-7 bg-white border-2 border-[#1A1A1A] p-4 sm:p-5 flex flex-col justify-between shadow-sm">
          
          <div>
            {/* Map Legend */}
            <div className="flex flex-wrap items-center justify-between gap-2 pb-3 mb-3 border-b-2 border-[#1A1A1A] text-[11px] font-mono">
              <div className="flex items-center space-x-3">
                <span className="flex items-center gap-1.5 text-[#D43F33] font-bold uppercase">
                  <span className="w-2.5 h-2.5 bg-[#D43F33]"></span>
            ...
            {/* GIS Map Canvas */}
            <div className="relative w-full h-80 sm:h-96 bg-[#1A1A1A] text-white p-4 overflow-hidden flex flex-col justify-between border-2 border-[#1A1A1A]">
              ...
              <div className="relative z-10 flex items-center justify-between">
                <div className="bg-white text-[#1A1A1A] border-2 border-[#1A1A1A] px-2.5 py-1 text-[10px] font-mono font-bold uppercase flex items-center gap-1">
                  <MapPin className="w-3.5 h-3.5 text-[#D43F33]" />
                  <span>Ward #{currentWard?.number} Boundary Sector</span>
                </div>
                <div className="text-[10px] font-mono bg-black/60 text-white px-2 py-0.5 border border-white/20">
              ...
              <div className="relative z-10 my-auto space-y-2 py-2 overflow-y-auto max-h-56 pr-1 no-scrollbar">
                {wardWorkOrders.map((wo) => {
                  ...
                  return (
                    <div
                      ...
                      className={`cursor-pointer transition-all p-2.5 border-2 flex items-center justify-between ${
                        isSelected
                          ? isBreach
                            ? 'bg-[#D43F33] text-white border-white'
                            : 'bg-white text-[#1A1A1A] border-white'
                          : isBreach
                          ? 'bg-[#1A1A1A] border-[#D43F33] text-white hover:bg-black'
                          : 'bg-[#1A1A1A] border-white/30 text-white/90 hover:border-white'
                      }`}
                    >
                      <div className="flex items-center space-x-2.5 min-w-0">
                        <div className={`w-2.5 h-2.5 shrink-0 ${
                          isBreach ? 'bg-[#D43F33]' : 'bg-emerald-400'
                        } ${isSelected ? 'ring-2 ring-white' : ''}`} />
                        <div className="min-w-0">
                          <p className="text-xs font-bold truncate uppercase tracking-tight">
                            {wo.roadName}
                          </p>
                          <p className={`text-[10px] font-mono flex items-center gap-1.5 truncate ${isSelected && isBreach ? 'text-white/90' : 'opacity-70'}`}>
                      ...
                      <div className="text-right shrink-0 pl-2">
                        <span className={`text-[10px] font-mono font-bold uppercase px-2 py-0.5 border ${
                          isSelected
                            ...
                            : isBreach
                            ? 'bg-[#D43F33]/20 text-[#D43F33] border-[#D43F33]'
                ...
                {wardWorkOrders.length === 0 && (
                  <div className="text-center py-8">
                    <p className="text-xs font-mono uppercase opacity-60">No road segments found matching filter.</p>
                  </div>
                )}
              </div>

              {/* Bottom Help Text */}
              <div className="relative z-10 flex items-center justify-between text-[10px] font-mono opacity-70 pt-2 border-t border-white/20">
                <span>State e-Procurement GIS Layer</span>
                <span className="font-bold uppercase text-[#D43F33]">Warranty: 36 Months Mandatory</span>
              </div>
        ...
        <div className="lg:col-span-5 bg-white border-2 border-[#1A1A1A] p-5 space-y-4 shadow-sm">
          ...
          {selectedWorkOrder ? (
            <>
              ...
              <div>
                <div className="flex items-center justify-between">
                  <span className="text-[10px] uppercase font-mono font-bold opacity-60">
                    Tender: {selectedWorkOrder.tenderNumber}
                  </span>
                  <span className={`text-[11px] font-mono font-bold uppercase px-2 py-0.5 ${
                    selectedWorkOrder.dlpStatus === 'DLP_BREACH_UNRESOLVED'
                      ? 'bg-[#D43F33] text-white'
                ...
                <h3 className="text-xl font-black uppercase tracking-tight text-[#1A1A1A] mt-1.5">
              ...
              {/* Contractor Card */}
              <div className="bg-[#FAFAFA] p-3.5 border-2 border-[#1A1A1A] space-y-2">
                <span className="text-[10px] uppercase font-mono tracking-widest opacity-60 font-bold block">
                ...
                <div className="flex items-center justify-between">
                  ...
                  {roadContractor && (
                    <span className={`text-[11px] font-mono font-bold uppercase px-2 py-0.5 ${
                      roadContractor.integrityScore < 40 ? 'bg-[#D43F33] text-white' : 'bg-[#1A1A1A] text-white'
                    }`}>
                      Score: {roadContractor.integrityScore}/100
                    </span>
                  )}
                </div>
                <div className="flex items-center justify-between text-xs font-mono text-[#1A1A1A]/80 border-t border-black/10 pt-1.5">
                  <span>Sanctioned: <strong className="text-[#1A1A1A]">₹{selectedWorkOrder.sanctionedAmountLakhs} Lakhs</strong></span>
                  <span>Grade: <strong className="text-[#1A1A1A]">{selectedWorkOrder.specifications.bitumenGrade}</strong></span>
                </div>
              </div>

              {/* Defect Liability Countdown Box */}
              <div className={`p-3.5 border-2 ${
                dlpDaysLeft > 0 
                  ? 'border-[#D43F33] bg-[#D43F33]/5 text-[#1A1A1A]' 
                  : 'border-[#1A1A1A] bg-[#FAFAFA] text-[#1A1A1A]'
              }`}>
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold uppercase tracking-tight flex items-center gap-1.5">
                    <Calendar className="w-3.5 h-3.5 text-[#D43F33]" />
                    Defect Liability Period (DLP)
                  </span>
                  <span className="text-xs font-mono font-bold bg-[#1A1A1A] text-white px-2 py-0.5 uppercase">
                    {dlpDaysLeft > 0 ? `${dlpDaysLeft} Days Left` : 'Expired'}
                  </span>
                </div>
                <p className="text-xs text-[#1A1A1A]/80 mt-1.5 font-serif">
                  Completed on {selectedWorkOrder.completionDate} • Warranty active till <strong>{selectedWorkOrder.dlpExpiryDate}</strong>.
                </p>
                <p className="text-[10px] text-[#D43F33] font-mono font-bold mt-1 uppercase">
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between text-xs font-bold uppercase">
                  ...
                  <span className="text-[10px] font-mono text-[#D43F33]">
                ...
                <div className="space-y-2 max-h-36 overflow-y-auto pr-1">
                  {roadDefects.map((d) => (
                    <div key={d.id} className="p-2.5 bg-[#FAFAFA] border border-[#1A1A1A] text-xs">
                      <div className="flex items-center justify-between">
                        <span className="font-bold text-[#D43F33] uppercase">{d.defectType}</span>
                        <span className="text-[10px] font-mono opacity-60">{d.reportedDate}</span>
                      </div>
                      <p className="text-xs font-serif text-[#1A1A1A]/80 mt-0.5">{d.description}</p>
                      <div className="mt-1 flex items-center justify-between text-[10px] font-mono opacity-70">
                        <span>By: {d.reporterName}</span>
                        <span className="font-bold text-[#1A1A1A]">{d.citizenUpvotes} verifications</span>
                      </div>
                    </div>
                  ))}

                  {roadDefects.length === 0 && (
                    <div className="p-3 bg-[#FAFAFA] border border-black/20 text-center text-xs font-mono opacity-60">
              ...
              <div className="pt-2 flex flex-col sm:flex-row items-center gap-2">
                <button
                  ...
                  className="w-full sm:flex-1 inline-flex items-center justify-center space-x-1.5 bg-white hover:bg-black/5 text-[#1A1A1A] text-xs font-bold uppercase px-3 py-2 border-2 border-[#1A1A1A] transition-colors"
                >
                  <PlusCircle className="w-3.5 h-3.5 text-[#D43F33]" />
                  <span>Log Defect Here</span>
                </button>

                <button
                  onClick={() => onGenerateActionWithWorkOrder(selectedWorkOrder)}
                  className="w-full sm:flex-1 inline-flex items-center justify-center space-x-1.5 bg-[#D43F33] hover:bg-[#1A1A1A] text-white text-xs font-bold uppercase tracking-tight px-3 py-2 transition-colors shadow-sm"
                >
            ...
            <div className="text-center py-16 font-mono text-xs opacity-60 uppercase">
