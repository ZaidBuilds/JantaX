    ...
    <div style={{ padding: '1.75rem 0', maxWidth: 1200, margin: '0 auto' }}>
      ...
      <div style={{ marginBottom: '1.5rem' }}>
        <div style={{ fontSize: '0.8rem', color: '#64748b', fontWeight: 600, marginBottom: '0.4rem' }}>
          <Link to="/rera" style={{ color: '#0891b2', textDecoration: 'none' }}>RERA Intelligence</Link> / Search & Filter
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(280px, 320px) 1fr', gap: '1.5rem' }}>
        {/* Filter Sidebar */}
        <div style={{ background: '#ffffff', borderRadius: 16, border: '1px solid #e2e8f0', padding: '1.25rem', height: 'fit-content' }}>
          <h2 style={{ fontSize: '1rem', fontWeight: 800, color: '#0f172a', marginTop: 0, marginBottom: '1rem' }}>
            Filter RERA Database
          </h2>

          <div style={{ marginBottom: '1rem' }}>
            <label style={{ display: 'block', fontSize: '0.78rem', fontWeight: 700, color: '#334155', marginBottom: '0.35rem' }}>Search Query</label>
            <input
              type="text"
          ...
          <div style={{ marginBottom: '1rem' }}>
            <label style={{ display: 'block', fontSize: '0.78rem', fontWeight: 700, color: '#334155', marginBottom: '0.35rem' }}>State RERA Portal</label>
            <select value={statePortal} onChange={(e) => setStatePortal(e.target.value)} style={{ width: '100%', padding: '0.55rem 0.75rem', borderRadius: 8, border: '1px solid #cbd5e1', fontSize: '0.85rem' }}>
              {portals.map((p) => <option key={p} value={p}>{p}</option>)}
            </select>
          </div>

          <div style={{ marginBottom: '1rem' }}>
            <label style={{ display: 'block', fontSize: '0.78rem', fontWeight: 700, color: '#334155', marginBottom: '0.35rem' }}>Status</label>
            <select value={status} onChange={(e) => setStatus(e.target.value)} style={{ width: '100%', padding: '0.55rem 0.75rem', borderRadius: 8, border: '1px solid #cbd5e1', fontSize: '0.85rem' }}>
              {statusOptions.map((st) => <option key={st} value={st}>{st}</option>)}
            </select>
          </div>

          <div>
            <label style={{ display: 'block', fontSize: '0.78rem', fontWeight: 700, color: '#334155', marginBottom: '0.35rem' }}>Sort By</label>
            <select value={sortBy} onChange={(e: any) => setSortBy(e.target.value)} style={{ width: '100%', padding: '0.55rem 0.75rem', borderRadius: 8, border: '1px solid #cbd5e1', fontSize: '0.85rem' }}>
        ...
        <div style={{ display: 'grid', gap: '1rem' }}>
          {filteredProjects.map((p) => (
            <div key={p.id} style={{ background: '#ffffff', borderRadius: 14, border: '1px solid #e2e8f0', padding: '1.25rem' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '0.5rem' }}>
                <div>
                  ...
                  <h3 onClick={() => navigate(`/rera/projects/${p.id}`)} style={{ fontSize: '1.15rem', fontWeight: 800, color: '#0f172a', cursor: 'pointer', margin: '0.3rem 0 0.15rem' }}>
                    {p.projectName}
                  </h3>
                ...
                <Link to={`/rera/projects/${p.id}`} style={{ fontSize: '0.8rem', color: '#0891b2', fontWeight: 700, textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: '0.2rem' }}>
