    ...
    <div style={{
      ...
      <div style={{ display: 'grid', gap: '1rem' }}>
        <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', alignItems: 'center' }}>
          <span style={{ background: 'var(--accent-solid)', color: 'var(--on-solid)', fontSize: '0.75rem', fontWeight: 800, padding: '0.2rem 0.65rem', borderRadius: 6 }}>
            Category: {category}
          </span>
          <span style={{ fontSize: '0.78rem', color: 'var(--ink-3)', fontWeight: 600, display: 'inline-flex', alignItems: 'center', gap: '0.25rem' }}>
            <MapPin size={13} style={{ color: 'var(--accent-ink)' }} /> PIN {pinCode} · {landmark}, {district}, {state}
          </span>
        </div>

        <div>
          <h4 style={{ fontSize: '1.15rem', fontWeight: 800, color: 'var(--ink)', margin: '0.2rem 0 0.4rem' }}>
            {title || 'Untitled Issue Report'}
          </h4>
          <p style={{ fontSize: '0.88rem', color: 'var(--ink-2)', margin: 0, lineHeight: 1.55 }}>
        ...
        <div>
          <div style={{ fontSize: '0.78rem', color: 'var(--ink-3)', fontWeight: 700, textTransform: 'uppercase', marginBottom: '0.4rem' }}>
            Attached Media Evidence ({evidenceFiles.length})
          </div>
          {evidenceFiles.length === 0 ? (
            <div style={{ fontSize: '0.82rem', color: 'var(--ink-4)' }}>No media attached.</div>
          ) : (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '0.75rem' }}>
              {evidenceFiles.map((file, i) => (
                <div key={i} style={{ borderRadius: 8, overflow: 'hidden', border: '1px solid var(--border-strong)', height: 90 }}>
        ...
        <div style={{
          ...
          <ShieldCheck size={15} style={{ color: 'var(--good)' }} />
